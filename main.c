...
// {
//     // Desenează întrebarea
//     draw_question(currentQuestion);

//     // Tratează inputul pentru răspuns (exemplu pentru tastele 1-4)
//     for (int i = 0; i < 4; i++) {
//         if (IsKeyPressed(KEY_ONE + i) || IsKeyPressed(KEY_KP_1 + i)) {
//             int result = (i == currentQuestion->data.correctAnswer) ? 1 : 0;
//             // Poți adăuga feedback aici dacă vrei
//             state = STATE_RUNNING;
//             food = spawn_food(snake);
//             foodJustEaten = false;
//         }
