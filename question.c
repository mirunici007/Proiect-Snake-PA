...
void DrawTextManualWrap(const char* text, int x, int y, int maxWidth, int fontSize, Color color) {
    ...
    while (*line) {
        ...
        DrawText(line, centerX, y, fontSize, color);
...
void draw_question(QuestionNode* q) {
    ...
    //DrawText(firstLine, screenWidth / 2 - MeasureText(firstLine, 22) / 2, 100, 22, DARKGRAY);
    DrawTextManualWrap(firstLine, screenWidth / 2 - 400, 100, 800, 22, RAYWHITE);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        DrawTextManualWrap(line, answerBoxes[i].x + 50, answerBoxes[i].y + 12, boxWidth - 60, 20, BLACK);
    }
